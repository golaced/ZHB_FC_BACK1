...
void NRF_Send_ARMED(void)//????
{
  ...
  NRF24L01_TXDATA[cnt++]=BYTE3(_temp);
	...
	NRF24L01_TXDATA[cnt++]=BYTE1(_temp);
	NRF24L01_TXDATA[cnt++]=BYTE0(_temp);
	_temp = (int)(state_v*100);//(int)(Roll*100);// (int)((Target.Yaw+180)*100);//
	NRF24L01_TXDATA[cnt++]=BYTE1(_temp);
	NRF24L01_TXDATA[cnt++]=BYTE0(_temp);
	}
  _temp = (int)(Yaw*100);
	NRF24L01_TXDATA[cnt++]=BYTE1(_temp);
	NRF24L01_TXDATA[cnt++]=BYTE0(_temp);
	_temp=Rc_Pwm_Out_mine[RC_THR]-1000;//(plane.get.altitude.sonar);//hight
	NRF24L01_TXDATA[cnt++]=BYTE1(_temp);
	NRF24L01_TXDATA[cnt++]=BYTE0(_temp);
...
//		{
	 ...
	 // _temp=bmp_speed;//(plane.get.altitude.sonar);//hight
		//else if(height_ctrl_mode==2)
	  if(mode.test3)
    _temp=ALT_POS_SONAR_HEAD*1000;
		else if(mode.test2)
		_temp=ALT_POS_SONAR_HEAD_LASER_SCANER*1000;	
    else		
...
{	vs16 _temp;	u8 _cnt=0;u8 i;u8 sum = 0;
	...
	DEBUG[3]=0;//nav_circle[0]*100;
	DEBUG[4]=0;//nav_circle[1]*100;	
	}
	else{
	DEBUG[1]=exp_height/10;
	DEBUG[2]=ultra_dis_lpf/10;
	DEBUG[3]=ultra_ctrl_out;
	DEBUG[4]=ultra_speed;	
